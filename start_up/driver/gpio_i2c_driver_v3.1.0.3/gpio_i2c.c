/*
 * $Header: /rd_2/project/SoC/Components/GPIOC/Device_Driver/GPIO_I2C/gpio_i2c.c 26    10/04/30 5:33p Evelyn $
 *
...
/* ======================================== */
static const CHAR GPIOI2C_DATE[] = "$Date: 10/04/30 5:33p $";
...
/* ======================================== */
            ...
            if (readl(GPIO_I2C_Reg + GPIOC_MMR_DATAIN) & GPIOI2C_INTR)
            {
	            ...
	            if (wait_event_interruptible(g_GPIO_I2C_devices->tWaitQueueHead,
	            	((g_GPIO_I2C_devices->GPIO_intr_occurred & GPIOI2C_INTR)!=0)) != 0)
	            {
...
/* ======================================== */
    ...
    g_GPIO_I2C_devices->GPIO_intr_occurred = status;
/* Version 3.0.0.16 modification, 2009.08.18 */
/* Version 3.1.0.1 modification, 2009.12.03 */
/* ======================================== */
/* ========================================= */
...
/* Version 3.1.0.0 modification, 2009.11.20 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
		ret = unregister_chrdev(g_GPIO_I2C_major, "gpio_i2c");
		if (ret < 0)
		{
			printk("ERROR: can't release major %d\n",
					g_GPIO_I2C_major);
			return;
		}
#else
...
/* Version 3.1.0.0 modification, 2009.11.20 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24)
    ret = request_irq(VPL_GPIOC_IRQ_NUM, &GPIO_I2C_ISR, SA_SHIRQ, "GPIO controller", g_GPIO_I2C_devices);
#else
...
MODULE_DESCRIPTION("I2C Driver Over GPIO on VN SoC");
MODULE_LICENSE("GPL");
/* Version 3.0.0.8 modification, 2009.04.01 */
//EXPORT_NO_SYMBOLS;
/* ======================================== */
